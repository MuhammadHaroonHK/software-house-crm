...
import { Calculator, Loader2, Pencil, Plus, Trash2, X } from "lucide-react";
...
export default function QuotationItemsEditor({
  ...
  const { data, isLoading, isFetching, isError, refetch } = useQuotationItems(
    quotation?.id,
  );
  ...
  const validate = () => {
    ...
    } else if (serviceName.length > 200) {
      nextErrors.serviceName = "Service name cannot exceed 200 characters.";
    }
  ...
  const handleDelete = async (item: QuotationItem) => {
    try {
        ...
        <div className="flex shrink-0 items-start justify-between border-b border-slate-200 px-6 py-5">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              ...
              {quotation.client?.companyName ?? "Client"}
            </p>
          </div>
