...
import { CreateInvoiceDTO, UpdateInvoiceDTO } from "./invoice.types";
...
export class InvoiceService {
  async create(data: CreateInvoiceDTO) {
    const quotation = await invoiceRepository.findQuotationForInvoice(
      data.quotationId,
    );
    ...
    const existingInvoice = await invoiceRepository.findByQuotationId(
      data.quotationId,
    );

    if (existingInvoice) {
      throw new AppError(409, "Invoice already exists for this quotation.");
    }
    ...
    const invoice = await invoiceRepository.createFromQuotation(
      data.quotationId,
      {
  ...
  async update(id: string, data: UpdateInvoiceDTO) {
    ...
    if (dueDate < invoice.issueDate) {
      throw new AppError(400, "Due date must be after issue date.");
    }
  ...
  async send(id: string) {
    ...
    const sentInvoice = await invoiceRepository.send(id);
